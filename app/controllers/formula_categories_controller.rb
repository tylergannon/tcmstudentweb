class FormulaCategoriesController < ApplicationController
  def index
		@formula_categories = FormulaCategory.find(:all, :conditions => ['name LIKE ?', "%#{params[:search]}%"], :order => "position")
  ...
  def sort
    params[:formula_categories].each_with_index do |id, index|
      FormulaCategory.update_all(['position=?', index+1], ['id=?', id])
    end
    render :nothing => true
  end
